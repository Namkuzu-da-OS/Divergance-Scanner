...
Review the current conversation context and write the checkpoint to the `## SESSION CHECKPOINT` section at the bottom of `data/MARKET_INTEL.md`.
...
Use the **Edit tool** to replace the content between `<!-- CHECKPOINT:START -->` and `<!-- CHECKPOINT:END -->` markers in `data/MARKET_INTEL.md`. The section lives at the very bottom of the file under `## SESSION CHECKPOINT`.

Replace EVERYTHING between (but not including) those two markers with this structure:
...
### KEY LEVELS
[SPY and QQQ key levels: call wall, put wall, gamma flip, SMAs, VWAP, max pain]

### CALENDAR (CONFIRMED ONLY)
[Only events confirmed by API data, user input, or verified web search]

### SESSION NOTES
[Any other important context from THIS SESSION's data. NEVER fabricate calendar dates or events.]
```
